...
const PrintCashReceipt: React.FC = () => {
  ...
  const [receiptData, setReceiptData] = useState<CashReceiptData | null>(null);
  ...
  // Extract the receipt ID from the URL query parameters
  const getReceiptId = () => {
  ...
  useEffect(() => {
    const validateReceipt = async () => {
      if (!receiptId) {
        ...
        setLoading(false);
        showToast('No receipt ID provided', 'error');
        return;
      }

      try {
        const response = await fetch(`${constants.baseURL}/json/cash-receipts`);
        if (!response.ok) {
          throw new Error('Failed to fetch receipt data');
        }

        const receipts = await response.json();
        const receipt = receipts.find((r: any) => r.id === parseInt(receiptId));
        ...
        const party = parties.find((p: any) => p.C_CODE === receipt.party);
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <PageMeta title="Error | Cash Receipt Print" description="Error loading cash receipt data" />
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <PageMeta title="Cash Receipt Print" description="Print cash receipt details" />
      ...
      <div className="border border-gray-300 p-8 rounded-lg">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">Receipt #{receiptData?.receiptNo}</p>
        </div>
        
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            <p><span className="font-semibold">Date:</span> {receiptData?.date}</p>
            <p><span className="font-semibold">Series:</span> {receiptData?.series}</p>
          </div>
          <div>
            <p><span className="font-semibold">Receipt No:</span> {receiptData?.receiptNo}</p>
            <p><span className="font-semibold">Party:</span> {receiptData?.partyName}</p>
          </div>
        </div>
        ...
        <div className="border-t border-gray-300 pt-4 mb-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <p><span className="font-semibold">Discount:</span> ₹{receiptData?.discount}</p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p><span className="font-semibold">Net Amount:</span> ₹{
                parseFloat(receiptData?.amount || '0') - parseFloat(receiptData?.discount || '0')
              }</p>
            </div>
          </div>
        </div>
