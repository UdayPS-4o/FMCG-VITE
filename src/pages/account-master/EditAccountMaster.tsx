...
// Interface for Input component error prop
...
const EditAccountMaster: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (account) {
          ...
          // Fetch GSTIN data if available
          if (account.gst && account.gst.length === 15) {
            fetchGSTINData(account.gst);
          }
        } else {
  ...
  const fetchGSTINData = async (gstin: string) => {
    if (gstin.length === 15) {
      setIsValidating({ ...isValidating, gstin: true });
      ...
      try {
        ...
        setIsValidating({ ...isValidating, gstin: false });
      }
    }
  };
    ...
    try {
      ...
      const response = await fetch(`${constants.baseURL}/account-master/update/${id}`, {
        ...
        body: JSON.stringify({
          code: values.achead,
          name: values.achead,
          gst: values.gstin || '', // Use gstin field but send as gst for API compatibility
