...
const CashPaymentApproved: React.FC = () => {
  ...
  useEffect(() => {
    const history = localStorage.getItem('cashPaymentSyncHistory');
    if (history) {
      try {
  ...
  const updateSyncHistory = (records: any[]) => {
    ...
    // Create transaction records for each synced payment
    ...
    localStorage.setItem('cashPaymentSyncHistory', JSON.stringify(updatedHistory));
  };
  ...
  const handleSyncToDbf = async () => {
    ...
    try {
      ...
      let response;
      try {
        response = await fetch(`${baseUrl}/api/merge/cash-payments/sync`, {
          method: 'POST',
          headers: {
        ...
        toast.success(`Successfully synced ${selectedRecords.length} cash payments (simulated)`);
      ...
      if (data.success) {
        updateSyncHistory(selectedRecords);
        toast.success(`Successfully synced ${data.syncedCount} cash payments`);
        ...
        toast.error(data.message || 'Failed to sync cash payments');
      }
    } catch (error) {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('fetch') || error.message.includes('network')) {
          errorMessage = 'API endpoint not available. The server needs to register the cash-payments merge route.';
          toast.error(errorMessage, { autoClose: false });
  ...
  const handleRevertSelected = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(`Successfully reverted ${selectedRecords.length} cash payments to database`);
        // Refresh the data
        if (tableRef.current) {
          await tableRef.current.refreshData();
        }
      } else {
        toast.error(data.message || 'Failed to revert cash payments');
      }
    } catch (error) {
      console.error('Error reverting cash payments:', error);
      toast.error('Error reverting cash payments. Check console for details.');
    } finally {
                    ...
                    {syncHistory.map((record, index) => (
                      <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                          ...
                          >
                            {record.transactions.length} payment(s) synced
                          </button>
                        </td>
                      </tr>
                    ))}
